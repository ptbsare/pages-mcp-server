...
export class PagesDatabase {
    ...
    async getPageByShareId(shareId) {
        const db = await this.ensureDb();
    ...
    async listPages(limit = 50, offset = 0) {
        ...
        while (pagesStmt.step()) {
            pages.push(this.rowToPage(pagesStmt.getAsObject()));
        }
        pagesStmt.free();
        return { pages, total };
    }
    async updatePage(id, updates) {
        ...
        check.bind([id]);
        check.step();
        const count = check.getAsObject().cnt;
        check.free();
        return count > 0;
    }
    async deletePage(id) {
    ...
    async createToken(token) {
        const db = await this.ensureDb();
        db.run(`INSERT INTO auth_tokens (id, token, name, created_at, last_used_at) VALUES (?, ?, ?, ?, ?)`, [token.id, token.token, token.name, token.createdAt, token.lastUsedAt ?? null]);
        this.save();
    }
    ...
    async listTokens() {
        ...
        const tokens = [];
        while (stmt.step()) {
            tokens.push(this.rowToAuthToken(stmt.getAsObject()));
        }
        stmt.free();
        return tokens;
    }
