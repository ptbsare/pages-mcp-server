...
export function createApp(config) {
    ...
    app.get("/f/:shareId/:fileName?", async (req, res) => {
        const { shareId } = req.params;
        if (!/^[a-zA-Z0-9_-]{1,64}$/.test(shareId)) {
        ...
        if (meta.type === "file") {
            ...
            if (isImage) {
                ...
                try {
                    ...
                    content = `<div class="preview"><div class="placeholder"><div class="icon">📄</div><p>Cannot preview this file</p></div></div>`;
                }
            }
            else {
    ...
    app.get("/f/:shareId/raw/:fileName?", async (req, res) => {
        const { shareId } = req.params;
        if (!/^[a-zA-Z0-9_-]{1,64}$/.test(shareId)) {
            res.status(400).send("Bad Request");
            return;
