    ...
    return {
      tools: [
        ...
        },
        {
          name: "deploy_folder",
          description:
            "Deploy a local folder containing a static website (must include index.html) to the remote server. Returns a shareable URL.",
          inputSchema: {
            type: "object",
            properties: {
              path: {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        ...
        case "deploy_folder": {
          const { path, name: pageName, description } = args as any;
